...
local function panorama()
  local function angleLook(yaw)
    local info = game:playerInfo()
    local pos = info.pos
    local angles = game:playerInfo().angles
    angles[2] = angles[2] + yaw
    local buffer = game:renderCustomView{
        width = SHAPE.width,
        height = SHAPE.height,
        pos = pos,                      --array of numbers
        look = angles,                    --array of numbers
        renderPlayer = false,
    }
    return buffer:clone()
  end
  local left    = angleLook(0)
  local forward = angleLook(90)
  local right   = angleLook(180)
  local back    = angleLook(270)

  local img_shape = left:shape()
  local panorama = tensor.ByteTensor(4, img_shape[1], img_shape[2], img_shape[3])
  panorama:select(1,1):copy(left)
  panorama:select(1,2):copy(forward)
  panorama:select(1,3):copy(right)
  panorama:select(1,4):copy(back)
  return panorama
end
